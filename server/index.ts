...
import { Game, GameStateData, PlayerState, CardData } from './game.js'; // ★ Gameクラスと型定義をインポート
...
const connectedPlayers: { [socketId: string]: string } = {}; // socketId -> playerId (今は同じ)
const games: { [playerId: string]: Game } = {}; // playerId -> Gameインスタンス
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('playCard', (cardInstanceId: string) => {
    const game = games[socket.id];
    if (!game) {
      socket.emit('message', 'ゲームが見つかりません。');
      return;
  ...
  // ★ 攻撃イベントのリスナーを追加
  socket.on('attack', (payload: { attackerId: string; targetId: string; targetType: 'follower' | 'leader' }) => {
    const game = games[socket.id];
    if (!game) {
      socket.emit('message', 'ゲームが見つかりません。');
      return;
